...
interface HandleConfirmAndPayProps {
...
export const handleConfirmAndPay = async ({
  reservation,
  selectedServices,
  roomId,
  navigate,
  ...
  try {
    ...
    const endDateTime = new Date(startDateTime.getTime() + reservation.hours * 60 * 60 * 1000)
    ...
    const selectedServicesArray = Array.from(selectedServices)
      .map((serviceId) => {
        ...
        for (const category in reservation.servicesByCategory) {
          const service = reservation.servicesByCategory[category as keyof ServicesByCategory]?.find(
            (s) => s.id === serviceId,
          )
          if (service) {
